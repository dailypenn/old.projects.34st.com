...
Save each project in its own folder with an `index.html` file to show up at `https://projects.34st.com/folder-name`. We standardized this by year and month, so make sure that you're creating pages in the correct folders for when they'll be published. (For example, the Fall Dining Guide should go in `[year]/10/dining-guide`).
...
### Projects Main Page
  1. All project pages should be linked on our [main projects page](https://projects.34st.com). To do this, edit the `projects.json` file at the root of this repo.
